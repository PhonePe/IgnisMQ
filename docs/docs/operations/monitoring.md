...
**Check:** `ignismq.handler.threads.active` against the handler pool's configured size, which derives
from `workerThreads`. The handler pool publishes no `threads.max` gauge of its own, and
...
(`rejected`), a throwing one (`exception`), a payload nobody could read (`unreadable`), timeouts,
saturation, and a sideline that would not accept the message at all (`sideline_refused`).

### 8. Unreadable payloads — `ignismq.sideline{reason=unreadable} > 0`

**Severity: high, and it usually means a deploy is half-done.** The stored payload could not be
turned into the consumer's message type, so **the handler never saw it**. This is not a handler fault:
the publisher and the consumer disagree about the format.

The common causes, in the order worth checking:

- A **partial deploy**: publishers are already writing the new shape and consumers have not rolled
  yet, or the reverse. Rate that starts at a deploy and stops at the rollback is diagnostic.
- A **field removed or renamed** without the consumer tolerating it.
- The **wrong message type** configured for the queue, in which case the whole queue fails rather
  than a fraction of it.

**Check:** the consumer's error log prints the payload alongside `retired as unreadable`. Compare it
with the publisher's current shape. If the rate matches the publish rate exactly, suspect the type;
if it is a fraction, suspect a rolling deploy.

!!! warning "These messages are in the sideline, and a shovel alone will not fix them"
    Replaying them before the format disagreement is resolved fails them again. Roll the consumer
    first, then shovel.

### 9. Sideline refused — `ignismq.sideline{reason=sideline_refused} > 0`
...
### 10. Storage call amplification — `magazine.aerospike.calls` per delivery, trending up
...
### 11. Control-pool starvation — `ignismq.pool.tasks.due{pool=control} > 0` sustained
...
### 12. Worker saturation — `ignismq.pool.threads{pool=worker}` at `ignismq.pool.threads.max`
